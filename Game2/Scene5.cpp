...
void Scene5::Init()
{
	...
	AcornWood1.SetLocalPosY(+330.0f);
...
void Scene5::Update()
{
	if (isChangingNext)
	{
		...
		{
			pl.SetWorldPos(Vector2(-200.0f, -150.0f));
		}
	}
	if (isChangingBack)
	{
		...
		if (LIGHT->lightColor.z < 0.1f)
		{
			pl.SetWorldPos(Vector2(-1170.0f, 700.0f));
		}
	}
